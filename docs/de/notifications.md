...
| **Lautstärke (0–100)** | Temporäre Ansage-Lautstärke. life360ng sendet den konfigurierten Wert direkt im Speak-Befehl (`volume;text`) und stellt den zuvor gelesenen `Player.volume`-Wert danach wieder her. |
...
> **Hinweis:** Bei gesetzter Lautstärke verwendet life360ng das Inline-Format `volume;text` für die Ansage. Zusätzlich wird der aktuelle `Player.volume`-Wert vorab gelesen und nach der Ansage wieder zurückgesetzt.
