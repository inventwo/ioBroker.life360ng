...
async function sendLocationNotification(personDpId, personFullName, newLocationName, locationType) {
	...
	// Build filtered recipient list: empty = all, otherwise match by display name (case-insensitive)
	const allowedNames = (personConfig.recipients || "")
		.split(",")
	...
	for (const recipient of notifyRecipients) {
		...
		if (allowedNames.length > 0) {
			...
			if (!allowedNames.includes(recipientName)) {
				continue;
			}
		}
		const instance =
