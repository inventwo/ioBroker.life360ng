...
async function publishPlace(place) {
    ...
    await setStateReadOnlyValue(`${dpPlace}.urlMapIframe`, "urlMapIframe", "string", "text.url", getMapsIframeUrl(lat, lng, 15), true);
}
...
async function publishPerson(person, idParentDp) {
    ...
    */
    ...
    await setStateReadOnlyValue(`${dpId}.disconnected`, "disconnected", "boolean", "indicator", Boolean(personIssues.disconnected), true);
    await setStateReadOnlyValue(`${dpId}.isConnected`, "isConnected", "boolean", "indicator.reachable", !Boolean(personIssues.disconnected), true);
    ...
    if (person.location) {
        ...
        if (track_location_people) {
            ...
            await setStateReadOnlyValue(`${dpId}.urlMapIframe`, "urlMapIframe", "string", "text.url", getMapsIframeUrl(lat, lng, 15), true);
        }

        //  Current place name from Life360
        await setStateReadOnlyValue(`${dpId}.locationName`, "locationName", "string", "text", person.location.name || "", true);
                    ...
                    for (let m in members) {
                        ...
                        try {
                            ...
                            if (member.location && member.location.sourceId) {
                                ...
                                if (Number(memberIssues.disconnected) == 0) memberEntered = (member.location.sourceId === place.id);
                            }
                            else {
                        ...
                        if (currentPeople[cp]._id.startsWith(`${adapter.namespace}.${dpPrefix.circles}.${circle.id}.places.${place.id}.`) && (currentPeople[cp]._id.split(".").length == 7)) {
                            if (!(members.some(myPerson => currentPeople[cp]._id.endsWith(myPerson.id)))) {
                                ...
                                try {
                                    ...
                                    adapter.log.warn(`Failed to get state for "${currentPeople[cp]._id}.timestamp": ${error}`);
                                    lastSeen = myTimestamp;
                                }
                                ...
                                if (diffDay > cleanupStaleDatapointsAge) {
                                    try {
                                        ...
                                        adapter.log.warn(`Failed to remove "${currentPeople[cp].common.name}" from Life360 circle "${circle.name}": ${error}`);
                                    }
                                }
                            }
                        }
                ...
                try {
                    ...
                    if (diffDay > cleanupStaleDatapointsAge) {
                        try {
                            ...
                            adapter.log.warn(`Failed to remove "${currentPlaces[i].common.name}" from Life360 circle "${circle.name}": ${error}`);
                        }
                    }
                } catch (error) {
                    adapter.log.warn(`Exception occured while cleaning up place id "${currentPlaces[i]._id}" from Life360 circle "${circle.name}": ${error}`);
                }
            }
        }
    }
...
async function publishMyPlaces(places, persons) {
    ...
    for (const p in places) {
        ...
        await setStateReadOnlyValue(`${dpPlaceInfo}.urlMapIframe`, "urlMapIframe", "string", "text.url", getMapsIframeUrl(mpLat, mpLng, 15), true);
