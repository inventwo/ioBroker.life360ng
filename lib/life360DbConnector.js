					...
					if (delay > 0) {
						if (memberIsAtPlace) {
							...
							if (!lastPresence && !isPresentDelayTimers.has(dpMember)) {
								...
								const timerMemberDpId = `${adapter.namespace}.${dpPrefix.people}.${member.id}`;
							...
							if (isPresentDelayTimers.has(dpMember)) {
								...
								isPresentDelayTimers.delete(dpMember);
								const cancelledDpId = `${adapter.namespace}.${dpPrefix.people}.${member.id}`;
								...
								if (!locationNameActive.has(cancelledDpId)) {
									await setStateReadOnlyValue(
										`${dpPrefix.people}.${member.id}.locationName`,
										"locationName",
										"string",
										"text",
							...
							if (lastPresence) {
								...
								const memberDpId = `${adapter.namespace}.${dpPrefix.people}.${member.id}`;
						...
						if (lastPresence != memberIsAtPlace) {
							...
							if (memberIsAtPlace) {
								...
								await sendLocationNotification(
									`${adapter.namespace}.${dpPrefix.people}.${member.id}`,
									`${member.firstName} ${member.lastName}`,
									myPlace.name,
									"myplace",
								);
