...
/** Per-device Alexa send queues to prevent rapid consecutive speak commands from overwriting each other */
const alexaSpeakQueues = new Map();

const ALEXA_MESSAGE_GAP_MS = 2000;
const ALEXA_INLINE_VOLUME_GAP_MS = 7000;
...
/**
 * @param {string} speakStateId Alexa2 speak state id
 * @returns {string|null} The sibling Player.volume state id, if derivable
 */
function getAlexaPlayerVolumeStateId(speakStateId) {
	return speakStateId.endsWith(".Commands.speak")
		? `${speakStateId.slice(0, -"Commands.speak".length)}Player.volume`
		: null;
}

/**
 * @param {string} message The text that will be spoken
 * @returns {number} Approximate time until the announcement has finished
 */
function getAlexaSpeechDurationMs(message) {
	const textLength = String(message || "").trim().length;
	return Math.min(15000, Math.max(3500, 1500 + textLength * 90));
}

/**
 * @param {object} device The configured Alexa target device
 * @param {string} message The text to speak
 * @returns {Promise<{usedInlineVolumeFallback: boolean}>} How the Alexa command was sent
 */
async function sendAlexaSpeakCommand(device, message) {
	const hasConfiguredVolume = device.volume !== undefined && device.volume !== null && device.volume !== "";
	const volume = hasConfiguredVolume ? Number(device.volume) : null;
	const playerVolumeStateId = getAlexaPlayerVolumeStateId(device.speakStateId);
	let originalVolume = null;
	let canRestoreVolume = false;

	if (hasConfiguredVolume && Number.isFinite(volume) && volume >= 0 && playerVolumeStateId) {
		try {
			const currentVolumeState = await adapter.getForeignStateAsync(playerVolumeStateId);
			if (!currentVolumeState || currentVolumeState.val === null || currentVolumeState.val === undefined) {
				throw new Error("current player volume is unavailable");
			}

			originalVolume = Number(currentVolumeState.val);
			if (!Number.isFinite(originalVolume)) {
				throw new Error(`current player volume is not numeric: ${currentVolumeState.val}`);
			}
			canRestoreVolume = true;
		} catch (error) {
			adapter.log.warn(
				`Notification: Could not read current Alexa player volume ${playerVolumeStateId} for ${device.speakStateId}: ${error.message}. Speech volume restore might be unavailable.`,
			);
		}
	}

	const speakText = hasConfiguredVolume && Number.isFinite(volume) && volume > 0 ? `${volume};${message}` : message;
	await adapter.setForeignStateAsync(device.speakStateId, speakText);
	await adapter.delay(getAlexaSpeechDurationMs(message));

	if (canRestoreVolume && originalVolume !== null && Number.isFinite(originalVolume) && originalVolume !== volume) {
		try {
			await adapter.setForeignStateAsync(playerVolumeStateId, originalVolume);
		} catch (error) {
			adapter.log.warn(
				`Notification: Failed to restore Alexa player volume ${playerVolumeStateId} to ${originalVolume}: ${error.message}`,
			);
		}
	}

	adapter.log.debug(
		`Notification: Sent Alexa speak to ${device.label || device.speakStateId}: "${speakText}" using inline configured volume${canRestoreVolume ? ` (restored ${playerVolumeStateId} to ${originalVolume})` : ""}`,
	);
	return { usedInlineVolumeFallback: hasConfiguredVolume && Number.isFinite(volume) && volume > 0 };
}

/**
 * @param {object} device The configured Alexa target device
 * @param {string} message The text to speak
 * @returns {Promise<void>} Resolves after the queued Alexa announcement and gap
 */
async function enqueueAlexaSpeak(device, message) {
	const queueKey = device.speakStateId;
	const currentQueue = alexaSpeakQueues.get(queueKey) || Promise.resolve();
	const nextQueue = currentQueue
		.catch(() => undefined)
		.then(async () => {
			try {
				const result = await sendAlexaSpeakCommand(device, message);
				await adapter.delay(
					result.usedInlineVolumeFallback ? ALEXA_INLINE_VOLUME_GAP_MS : ALEXA_MESSAGE_GAP_MS,
				);
			} catch (error) {
				adapter.log.warn(
	...
	alexaSpeakQueues.set(queueKey, nextQueue);
	try {
		await nextQueue;
	} finally {
		if (alexaSpeakQueues.get(queueKey) === nextQueue) {
			alexaSpeakQueues.delete(queueKey);
		}
	}
}
...
/**
 * Creates the notifications channel and its states in the ioBroker DB.
 * Called once at adapter startup before polling begins.
 * The state is always created regardless of whether Telegram or Alexa is enabled,
 * so it can be used independently in Blockly scripts or other automations.
 *
 * @returns {Promise<void>}
 */
exports.initNotificationStates = async function () {
	try {
		await createObjectDP(dpPrefix.notifications, "channel", "Notifications", "Notification output states");
		await createStateDP(
			`${dpPrefix.notifications}.lastSpokenText`,
			"lastSpokenText",
			true,
			false,
			"string",
		...
		const existing = await adapter.getStateAsync(`${dpPrefix.notifications}.lastSpokenText`);
		if (!existing) {
			await adapter.setStateAsync(`${dpPrefix.notifications}.lastSpokenText`, "", true);
		}
	} catch (error) {
		adapter.log.error(`Failed to initialize notification states: ${error}`);
	}
};
...
exports.sendTestAlexa = async function () {
	...
	const testMessages = ["Life360ng test notification one", "Life360ng test notification two"];
	let sent = 0;
	const errors = [];
	...
	for (const device of notifyAlexaDevices) {
		...
		try {
			for (const testMessage of testMessages) {
				await enqueueAlexaSpeak(device, testMessage);
			}
	...
	return { result: `Two-step test sent to ${sent} device(s)` };
};
		...
		if (cloud_data && cloud_data.circles) {
			...
			if (cloud_data.circles.length > 0) {
				...
				if (cloud_data.persons.length > 0) {
					...
					//  Log a summary after the first successful poll
					if (!firstPollDone) {
						firstPollDone = true;
						const placeCount = cloud_data.places ? cloud_data.places.length : 0;
						const myPlaceCount = Array.isArray(myPlaces) ? myPlaces.length : 0;
						myLogger.info(
							`Life360 data: ${cloud_data.circles.length} circle(s), ` +
								`${placeCount} Life360 place(s), ` +
								`${myPlaceCount} own place(s), ` +
								`${cloud_data.persons.length} person(s)`,
						);
					}

					//  Verbose logging (enabled in adapter config)
					if (debugging_verbose) {
						for (const c of cloud_data.circles) {
							myLogger.info(
								`[verbose] Circle "${c.name}": ` +
									`${(c.members || []).length} member(s), ` +
									`${(c.places || []).length} place(s)`,
							);
						}
						for (const place of cloud_data.places) {
							myLogger.info(`[verbose] Life360 place: "${place.name}" (radius: ${place.radius} m)`);
						}
						for (const person of cloud_data.persons) {
							const loc = person.location;
							const locStr = loc
								? `lat=${loc.latitude}, lon=${loc.longitude}${loc.name ? `, place="${loc.name}"` : ""}`
								: "no location";
							myLogger.info(
								`[verbose] Person: ${person.firstName} ${person.lastName} — ` +
									`battery=${person.location ? person.location.battery : "?"}%, ` +
									`disconnected=${Number((person.issues || {}).disconnected) !== 0}, ` +
									`${locStr}`,
							);
						}
					}
...
async function _dispatchNotification(message, personConfig) {
	// Always write spoken text to state for use in Blockly/automation (e.g. Sonos)
	await setStateReadOnlyValue(
		`${dpPrefix.notifications}.lastSpokenText`,
		"lastSpokenText",
		"string",
	...
	if (notifyAlexaEnabled && notifyAlexaDevices.length > 0) {
		for (const device of notifyAlexaDevices) {
			...
			await enqueueAlexaSpeak(device, message);
		}
	}
}

/**
 * @param {object} personConfig The person's notification config
 * @param {string} locationName The resolved location name
 * @param {'myplace'|'appplace'|'unknown'} locationType The current location type
 * @returns {string|null} The standard message for this location change, or null if disabled
 */
function buildStandardLocationMessage(personConfig, locationName, locationType) {
	...
	if (locationType === "unknown") {
		if (!personConfig.notifyUnknownPlaces) {
			return null;
		}
		return (personConfig.unknownPlacesMessage || "").trim() || null;
	}

	const rawPrefix = (personConfig.prefixText || "").trim();
	return rawPrefix ? `${rawPrefix} ${locationName}` : locationName;
}
...
async function sendLocationNotification(personDpId, personFullName, newLocationName, locationType) {
	...
	const messages = [];
	let suppressStandardMessage = false;

	// --- Leave notification for the previous place ---
	if (lastKnown && lastKnown !== "__unknown__") {
		...
		if (leaveOverride && leaveOverride.shouldNotify && leaveOverride.text) {
			messages.push(leaveOverride.text);
			if (leaveOverride.suppress) {
				suppressStandardMessage = true;
			}
		}
	}

	const standardMessage = buildStandardLocationMessage(personConfig, newLocationName, locationType);
	...
	let arrivalCustomMessage = null;

	if (arrivalOverride) {
		if (arrivalOverride.shouldNotify && arrivalOverride.text) {
			arrivalCustomMessage = arrivalOverride.text;
		}
		// suppress=true only disables the standard message when a custom arrival text exists.
		if (arrivalOverride.suppress && arrivalCustomMessage) {
			suppressStandardMessage = true;
		}
	}

	if (!suppressStandardMessage && standardMessage) {
		messages.push(standardMessage);
	}

	if (arrivalCustomMessage) {
		messages.push(arrivalCustomMessage);
	}
...
async function publishPerson(person, idParentDp) {
	...
	*/
	...
	await createObjectDP(
		...
		iobHelpers.resolveDisplayName(`${person.firstName} ${person.lastName}`, adapter.config.people_aliases),
		iobHelpers.resolveDisplayName(`${person.firstName} ${person.lastName}`, adapter.config.people_aliases),
	);
	...
	await setStateReadOnlyValue(
		...
		"boolean",
		"indicator",
		Number(personIssues.disconnected) !== 0,
		true,
	);
	await setStateReadOnlyValue(
		`${dpId}.isConnected`,
					...
					if (delay > 0) {
						if (memberIsAtPlace) {
							...
							if (!lastPresence && !isPresentDelayTimers.has(dpMember)) {
								...
								const timer = adapter.setTimeout(async () => {
							...
							if (isPresentDelayTimers.has(dpMember)) {
								...
								adapter.clearTimeout(isPresentDelayTimers.get(dpMember));
								isPresentDelayTimers.delete(dpMember);
