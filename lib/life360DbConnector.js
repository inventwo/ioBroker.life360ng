...
/** Global prefix text for unknown-location notifications */
let notifyUnknownEnabled = false;
let notifyUnknownPrefix = "";
...
exports.setAdapter = function (adapter_in) {
	...
	notifyUnknownEnabled = adapter.config.notify_unknown_enabled || false;
	notifyUnknownPrefix = (adapter.config.notify_unknown_prefix || "").trim();
};
...
/**
 * Sends Telegram notifications when a person arrives at a known location or an unknown location.
...
async function sendLocationNotification(personDpId, personFullName, newLocationName, locationType) {
	...
	if (locationType === "unknown" && !notifyUnknownEnabled) {
		return;
	}
