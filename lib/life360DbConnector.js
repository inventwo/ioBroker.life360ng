...
/**
 * Sends a test Telegram notification using the current unsaved admin form data.
 *
 * @param {object} config - Config object passed from the admin UI sendTo button
 * @returns {Promise<{result?: string, error?: string}>}
 */
exports.sendTestTelegram = async function (config) {
	if (!config || !config.notify_telegram_enabled) {
		return { error: "Telegram notifications are disabled" };
	}
	const recipients = Array.isArray(config.notify_recipients) ? config.notify_recipients : [];
...
/**
 * Sends a test Alexa announcement using the current unsaved admin form data.
 *
 * @param {object} config - Config object passed from the admin UI sendTo button
 * @returns {Promise<{result?: string, error?: string}>}
 */
exports.sendTestAlexa = async function (config) {
	if (!config || !config.notify_alexa_enabled) {
		return { error: "Alexa notifications are disabled" };
	}
	const devices = Array.isArray(config.notify_alexa_devices) ? config.notify_alexa_devices : [];
	if (devices.length === 0) {
		return { error: "No Alexa devices configured" };
	}
...
/**
 * Sends Telegram notifications when a person arrives at a known location.
 * Only fires when the locationName actually changed. Unknown/empty locations are ignored.
 *
 ...
 * @param {'myplace'|'appplace'} locationType Whether this is a custom place or a Life360 app place
 */
async function sendLocationNotification(personDpId, personFullName, newLocationName, locationType) {
	...
	if (!newLocationName || newLocationName === location_unknown_name) {
		return;
	}
	...
	if (lastKnown === undefined) {
		// First run – record value but don't notify to avoid sending on adapter start
	...
	const rawPrefix =
		locationType === "myplace"
			? (personConfig.prefixTextMyPlaces || personConfig.prefixText || "").trim()
			: (personConfig.prefixTextAppPlaces || personConfig.prefixText || "").trim();
	const message = rawPrefix ? `${rawPrefix} ${newLocationName}` : newLocationName;

	// Send Telegram notifications
	if (notifyTelegramEnabled) {
		// Build filtered recipient list: empty = all, otherwise match by display name or Chat ID (case-insensitive)
	...
	*/
	...
	if (person.location) {
		...
		if (!locationNameActive.has(dpId) && !locationNamePendingDelay.has(dpId)) {
			...
			// Notify Telegram when person arrives at a Life360 app place
