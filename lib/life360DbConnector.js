...
const ALEXA_VOLUME_SETTLE_MS = 750;
...
async function sendAlexaSpeakCommand(device, message) {
	...
	if (hasConfiguredVolume && Number.isFinite(volume) && volume >= 0 && playerVolumeStateId) {
		...
		try {
			...
			if (originalVolume !== volume) {
				await adapter.setForeignStateAsync(playerVolumeStateId, volume);
				changedVolume = true;
				await wait(ALEXA_VOLUME_SETTLE_MS);
			}

			await adapter.setForeignStateAsync(device.speakStateId, message);
			...
			adapter.log.debug(
				`Notification: Sent Alexa speak to ${device.label || device.speakStateId}: "${message}" using ${playerVolumeStateId} ${originalVolume} -> ${volume}${changedVolume ? ` -> ${originalVolume}` : ""}`,
			);
			return { usedInlineVolumeFallback: false };
		} catch (error) {
			if (changedVolume && Number.isFinite(originalVolume)) {
				try {
					await adapter.setForeignStateAsync(playerVolumeStateId, originalVolume);
				} catch {
					// Ignore restore errors here and continue with fallback logging below.
				}
			}
			adapter.log.warn(
				`Notification: Failed to use explicit Alexa player volume ${playerVolumeStateId} for ${device.speakStateId}: ${error.message}. Falling back to inline volume.`,
			);
		}
	}
	...
	adapter.log.debug(
		`Notification: Sent Alexa speak to ${device.label || device.speakStateId}: "${speakText}" using inline volume fallback`,
	);
