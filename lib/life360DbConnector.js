...
async function sendLocationNotification(personDpId, personFullName, newLocationName, locationType) {
	...
	if (lastKnown === undefined) {
		...
		// Read the previous locationName from DB to detect genuine transitions
		// that happened while the adapter was offline, instead of blindly suppressing.
		let previousKey = newLocationName; // fallback: treat as no change (suppress)
		try {
			const state = await adapter.getStateAsync(`${personDpId}.locationName`);
			if (state && state.val !== null && state.val !== undefined) {
				const displayVal = String(state.val);
				// Map display value back to internal notification key
				const isUnknown = !displayVal || displayVal === location_unknown_name;
				previousKey = isUnknown ? "__unknown__" : displayVal;
			}
		} catch {
			// DB error or state not yet created → keep fallback (suppress)
		}
		if (previousKey === newLocationName) {
			lastLocationNames.set(personDpId, newLocationName);
			return;
		}
		// Genuine transition that occurred while the adapter was offline → fall through to notify
	} else if (lastKnown === newLocationName) {
		return;
	}
	lastLocationNames.set(personDpId, newLocationName);
