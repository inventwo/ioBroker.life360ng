 ...
 * @returns {string|null} The sibling speakvolume state id, if derivable
 */
function getAlexaSpeakVolumeStateId(speakStateId) {
	return speakStateId.endsWith(".speak") ? `${speakStateId.slice(0, -"speak".length)}speakvolume` : null;
}
...
async function sendAlexaSpeakCommand(device, message) {
	...
	const speakVolumeStateId = getAlexaSpeakVolumeStateId(device.speakStateId);

	if (hasConfiguredVolume && Number.isFinite(volume) && volume >= 0 && speakVolumeStateId) {
		try {
			await adapter.setForeignStateAsync(speakVolumeStateId, volume);
			await adapter.setForeignStateAsync(device.speakStateId, message);
			adapter.log.debug(
				`Notification: Sent Alexa speak to ${device.label || device.speakStateId}: "${message}" using ${speakVolumeStateId}=${volume}`,
			);
			return { usedInlineVolumeFallback: false };
		} catch (error) {
			adapter.log.warn(
				`Notification: Failed to use Alexa speakvolume ${speakVolumeStateId} for ${device.speakStateId}: ${error.message}. Falling back to inline volume.`,
			);
		}
	}
