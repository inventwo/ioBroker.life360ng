...
class Tracker {
	...
	async _updateFamilyMap() {
		...
		const html = String(this._generateFamilyHTML(personFCs));
  ...
  setTimeout(function() {
	...
	async _generateFamilyHTML(personFCs) {
		...
		try {
			...
			if (circleId) {
				const ns = this.config.namespace;
				const state = await this.adapter.getStateAsync(`${ns}.circles.${circleId}.name`);
				if (state && typeof state.val === "string" && state.val.trim()) {
					circleName = state.val.trim();
				}
			}
		} catch (e) {
			this.adapter.log.debug(`[Tracker] Could not read circle name: ${e.message}`);
		}
