...
<link rel="stylesheet" href="/${ns}/tracker/tracker-map.css"/>
<style>:root{--page-bg:${c.pageBg};--header-bg:${c.headerBg};--header-border:${c.headerBorder};--fg:${headerFg};--ctrl-bg:${controlBg};--ctrl-border:${controlBorder};--ctrl-hover:${controlHoverBg}}body,input[type=date]{color-scheme:${colorScheme}}</style>
</head>
<body>
<div id="header">
	<h2>\uD83D\uDCCD ${firstName}</h2>
...
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></` +
			`script>
<script src="/${ns}/tracker/tracker-map.js"></` +
			`script>
</body>
</html>`
		);
...
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<link rel="stylesheet" href="/${ns}/tracker/tracker-map.css"/>
<style>:root{--page-bg:${c.pageBg};--header-bg:${c.headerBg};--header-border:${c.headerBorder};--fg:${headerFg};--ctrl-bg:${controlBg};--ctrl-border:${controlBorder};--ctrl-hover:${controlHoverBg};--sub-text:${subText}}body,input[type=date]{color-scheme:${colorScheme}}</style>
</head>
<body>
<div id="header">
	<h2>\uD83D\uDC68\u200D\uD83D\uDC69\u200D\uD83D\uDC67 ${circleName}</h2>
...
#header{padding:8px 14px;background:var(--header-bg,#16213e);display:flex;align-items:center;justify-content:space-between;font-size:13px;border-bottom:2px solid var(--header-border,#0f3460);flex-wrap:wrap;gap:6px;min-height:44px}
#header h2{font-size:15px;color:var(--fg,#f5f5f5);margin:0;white-space:nowrap}
#header-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
#headerInfo{color:var(--fg,#f5f5f5)}
...
#hamBtn{background:none;border:1px solid var(--ctrl-border);border-radius:4px;color:var(--fg,#f5f5f5);font-size:18px;line-height:1;padding:2px 8px;cursor:pointer;margin-left:10px}
...
function flagIcon(color,size){
  ...
  return L.divIcon({className:'',html:'<svg xmlns="http://www.w3.org/2000/svg" width="'+w+'" height="'+h+'" viewBox="0 0 24 24"><path paint-order="stroke fill" fill="'+color+'" stroke="#fff" stroke-width="1.5" stroke-linejoin="round" d="M14.4,6L14,4H5V21H7V14H12.6L13,16H20V6H14.4Z"/></svg>',iconSize:[w,h],iconAnchor:[0,h],popupAnchor:[Math.round(w/2),-h]});
}
...
function renderPlaceFlags(){
  ...
  (D.placesData||[]).forEach(function(p){flagLayers.push(L.marker([p.lat,p.lng],{icon:flagIcon(C.placesColor,C.placesFlagSize)}).bindPopup('<b>'+p.name+'</b>').addTo(map));});
}
function renderMyPlaceFlags(){
  ...
  (D.myPlacesData||[]).forEach(function(p){myFlagLayers.push(L.marker([p.lat,p.lng],{icon:flagIcon(C.myPlacesColor,C.myPlacesFlagSize)}).bindPopup('<b>'+p.name+'</b>').addTo(map));});
}
...
function personShowRange(from,to){
  ...
  feats.forEach(function(feat){
    ...
    layers.push(L.polyline(coords,{color:fc,weight:C.routeWeight||4,opacity:C.routeOpacity||0.85}).addTo(map));
    bounds.push.apply(bounds,coords);
    layers.push(L.circleMarker(coords[0],{radius:7,fillColor:dark,color:'#fff',weight:2,fillOpacity:1}).bindPopup(feat.date+'<br>\\u25b6 '+fmt(ts[0])).addTo(map));
    if(feat.date===maxDate){layers.push(L.marker(coords[coords.length-1],{icon:pinIcon(fc)}).bindPopup(feat.date+'<br>\\uD83D\\uDCCD '+fmt(ts[ts.length-1])).addTo(map));}
    else{layers.push(L.circleMarker(coords[coords.length-1],{radius:8,fillColor:fc,color:'#fff',weight:2,fillOpacity:C.markerOpacity||1}).bindPopup(feat.date+'<br>\\uD83D\\uDCCD '+fmt(ts[ts.length-1])).addTo(map));}
    coords.forEach(function(coord,i){if(i===0||i===coords.length-1)return;layers.push(L.circleMarker(coord,{radius:4,fillColor:fc,color:'#fff',weight:1,fillOpacity:0.6}).bindPopup(feat.date+'<br>'+fmt(ts[i])).addTo(map));});
  });
...
// ── Circle map ───────────────────────────────────────────
var visiblePeople={};
(D.people||[]).forEach(function(p){visiblePeople[p.name]=true;});
...
function circleRender(feats,withDate){
  ...
  feats.forEach(function(feat){
    ...
    if(showRoute){layers.push(L.polyline(coords,{color:color,weight:C.routeWeight||4,opacity:C.routeOpacity||0.85}).addTo(map));bounds.push.apply(bounds,coords);}
    if(showRoute&&coords.length>1){layers.push(L.circleMarker(coords[0],{radius:7,fillColor:dark,color:'#fff',weight:2,fillOpacity:1}).bindPopup(prefix+'\\u25b6 '+fmt(ts[0])).addTo(map));}
    if(feat.date===maxByPerson[feat.name]){layers.push(L.marker(coords[coords.length-1],{icon:pinIcon(color)}).bindPopup(prefix+'\\uD83D\\uDCCD '+fmt(ts[ts.length-1])).addTo(map));}
    else{layers.push(L.circleMarker(coords[coords.length-1],{radius:8,fillColor:color,color:'#fff',weight:2,fillOpacity:C.markerOpacity||1}).bindPopup(prefix+'\\uD83D\\uDCCD '+fmt(ts[ts.length-1])).addTo(map));}
    if(showRoute){coords.forEach(function(coord,i){if(i===0||i===coords.length-1)return;layers.push(L.circleMarker(coord,{radius:4,fillColor:color,color:'#fff',weight:1,fillOpacity:0.6}).bindPopup(prefix+fmt(ts[i])).addTo(map));});}
  });
...
if(hamBtn&&hamPanel){
  hamBtn.addEventListener('click',function(e){e.stopPropagation();hamPanel.style.display=hamPanel.style.display==='block'?'none':'block';});
...
if(menuRoute){
  ...
  function applyRoute(show){
    if(D.mode==='circle'){
      ...
      if(hr)hr.style.display=show?'flex':'none';
...
if(C.showPlaces){
  ...
  if(menuPlaces){
    ...
    menuPlaces.addEventListener('change',function(){localStorage.setItem('tracker_places_'+KEY,this.checked?'true':'false');renderPlaceFlags();});
  }
}
if(C.showMyPlaces){
  ...
  if(menuMyPlaces){
    ...
    menuMyPlaces.addEventListener('change',function(){localStorage.setItem('tracker_myplaces_'+KEY,this.checked?'true':'false');renderMyPlaceFlags();});
  }
}
renderPlaceFlags();renderMyPlaceFlags();
...
if(D.mode==='circle'){
  ...
  if(showAllBtn){showAllBtn.addEventListener('click',function(){document.querySelectorAll('.personToggle').forEach(function(cb){cb.checked=true;visiblePeople[cb.value]=true;});refreshCircle();});}
  if(hideAllBtn){hideAllBtn.addEventListener('click',function(){document.querySelectorAll('.personToggle').forEach(function(cb){cb.checked=false;visiblePeople[cb.value]=false;});refreshCircle();});}
}
