...
class Tracker {
	...
	_configStateDefs() {
		...
		return [
			...
			},
			{
				...
				value: mc.headerText,
				meta: { name: "Map - header text", type: "string", role: "level.color.hex", read: true, write: true },
			},
			{
				id: "tracker.config.color.routeWeight",
				value: mc.routeWeight,
				meta: {
					name: "Map - route weight (px)",
					type: "number",
					role: "value",
					min: 1,
					max: 10,
	 ...
	 */
	_writeMap(person, fc) {
		const html = String(this._generateHTML(person, fc));
		const paths = this._getPaths(person.name);
...
<script>
  ...
  setTimeout(function() {
	 ...
	 * @returns {Promise<string>} Complete HTML string of the Leaflet family map with dropdown and legend
	 */
	async _generateFamilyHTML(personFCs) {
		...
		// --- Circle Name aus Datenpunkt lesen ---
		let circleName = "Family";
		try {
			// Versuche, die Circle-ID aus der ersten Person zu nehmen
			...
			if (!circleId && this.config.people?.length > 0) {
				// Fallback: Versuche, die Circle-ID aus der ersten Person in der Config zu nehmen
