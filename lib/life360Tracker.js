...
class Tracker {
	...
	_writeMap(person, fc) {
		...
		const html = String(this._generateHTML(person, fc, person.ownMap !== false));
		const paths = this._getPaths(person.name);
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #header h2 { font-size:15px; color:${c.headerText}; margin:0; white-space:nowrap; }
	#header-right { display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-left:auto; }
	.route-checkbox-label { display:inline-flex; align-items:center; gap:4px; margin-left:12px; user-select:none; }
	.route-checkbox-label input[type="checkbox"] { vertical-align:middle; accent-color:${c.headerText}; margin:0; }
  ...
  input[type=date] {
	...
	font-size:13px;
	cursor:pointer;
	color-scheme:${c.headerText === "#111111" ? "light" : "dark"};
  }
  #map { flex:1; }
</style>
</head>
...
<script>
	...
	const STORAGE_KEY = "tracker_showRoute_${safeName}";
  ...
  if (location.hash) {
	...
	function showRange(from, to) {
		...
		feats.forEach(function(feat) {
			...
			layers.push(
				L.circleMarker(coords[coords.length-1], { radius:9, fillColor:fc, color:'#fff', weight:2, fillOpacity:1 })
	...
	// Checkbox handler for route display
	const showRouteCheckbox = document.getElementById('showRoute');
	// Restore checkbox state from localStorage, default to true
	...
	// Initial state: hide date pickers if route is enabled
	if (!showRouteCheckbox.checked) {
		...
		// Show only last point if route is hidden
		showDay(inpTo.value);
	} else {
		// Show range if route is enabled
		showRange(initFrom, initTo);
	}

	showRouteCheckbox.addEventListener('change', function() {
		// Persist state per person
		localStorage.setItem(STORAGE_KEY, this.checked ? 'true' : 'false');
		// Hide date pickers if route is disabled
  ...
  setTimeout(function() {
	 ...
	 * @returns {Promise<string>} Gibt den vollständigen HTML-String der Leaflet-Familienkarte zurück.
	 */
	async _generateFamilyHTML(personFCs) {
		...
		// const controlHoverBg = this._scaleColor(c.headerBg, 0.72); // removed unused
		const controlBorder = this._scaleColor(c.headerBg, 0.62);
		const subText = c.headerText || this._scaleColor(headerFg, 0.75);

		// --- Family Map Header Name: user-defined, circle name, or fallback ---
		let circleName = "Family";
		if (
			...
			this.adapter.log.debug(`[Tracker] FamilyMap: using user-defined header name: ${circleName}`);
		} else {
			try {
				...
				this.adapter.log.debug(`[Tracker] FamilyMap: detected circleId = ${circleId}`);
				if (circleId) {
					const ns = this.config.namespace;
					const stateId = `${ns}.circles.${circleId}.name`;
					const state = await this.adapter.getStateAsync(stateId);
					this.adapter.log.debug(`[Tracker] FamilyMap: reading state ${stateId}, value: ${state?.val}`);
					if (state && typeof state.val === "string" && state.val.trim()) {
						circleName = state.val.trim();
					} else {
						this.adapter.log.warn(
							`[Tracker] FamilyMap: State ${stateId} empty or not found, using default 'Family'`,
						);
					}
				} else {
					this.adapter.log.warn(`[Tracker] FamilyMap: No circleId found, using default 'Family'`);
				}
			} catch (e) {
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #header h2 { font-size:15px; color:${headerFg}; margin:0; white-space:nowrap; }
	#header-right { display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-left:auto; }
	.route-checkbox-label { display:inline-flex; align-items:center; gap:4px; margin-left:12px; user-select:none; }
	.route-checkbox-label input[type="checkbox"] { vertical-align:middle; accent-color:${headerFg}; margin:0; }
  ...
  .range-label { color:${subText}; font-size:12px; white-space:nowrap; }
...
<div id="header">
	...
	<label class="route-checkbox-label">
		<input type="checkbox" id="showRoute"> Routen
	</label>
</div>
<div id="map"></div>
<script>
  ...
  const SEL_DATE = "${selDate}";
	...
	const STORAGE_KEY = "tracker_showRoute_family";
  ...
  function fmt(ts) {
	...
	function renderFeatures(feats, withDateOnly) {
		...
		feats.forEach(function(feat) {
			...
			layers.push(
				L.circleMarker(coords[coords.length - 1], { radius:9, fillColor:color, color:'#fff', weight:2, fillOpacity:1 })
					.bindPopup(prefix + '⬛ Last: ' + fmt(ts[ts.length - 1])).addTo(map)
			);
  ...
  inpTo.addEventListener('change', function() {
	...
	// Initial state: hide date pickers if route is disabled
	if (!showRouteCheckbox.checked) {
		document.getElementById('dateFrom').style.display = 'none';
		document.getElementById('dateTo').style.display = 'none';
		document.getElementById('labelFrom').style.display = 'none';
