...
class Tracker {
	...
	_configStateDefs() {
		...
		return [
			...
			{
				id: "tracker.config.retentionDays",
				value: this.config.retentionDays,
				meta: {
					name: "Aufbewahrung (Tage, 0 = unbegrenzt)",
					type: "number",
					role: "value",
					unit: "d",
					min: 0,
	...
	async _loadConfigFromStates() {
		...
		const retentionDays = await get("tracker.config.retentionDays");
		...
		if (retentionDays !== null) {
			this.config.retentionDays = Number(retentionDays);
		}
	...
	async _applyConfigStateChange(shortId, val) {
		...
		switch (shortId) {
			...
			case "tracker.config.retentionDays":
				this.config.retentionDays = Number(val);
				return; // no map re-render needed
	...
	async _initPerson(person) {
		...
		await this._pruneOldData(person);
	...
	_generateHTML(person, fc) {
		...
		const refresh = Number(this.config.pollInterval) + 10;
...
<script>
  ...
  const availDates = new Set(FEATURES.map(f => f.date));
  const sortedDates = [...availDates].sort();
  const sel = document.getElementById('daySelect');
  if (sortedDates.length > 0) {
    sel.min = sortedDates[0];
    sel.max = sortedDates[sortedDates.length - 1];
    sel.value = availDates.has(SEL_DATE) ? SEL_DATE : sortedDates[sortedDates.length - 1];
  }
  sel.addEventListener('change', function() {
    if (availDates.has(this.value)) showDay(this.value);
    else { this.style.outline = '2px solid #ff6666'; setTimeout(() => { this.style.outline = ''; }, 800); }
  });
  if (FEATURES.length > 0) showDay(sel.value);
...
<script>
  setTimeout(function() {
	...
	/**
	 * Removes GPS features older than retentionDays from allTime GeoJSON.
	 * Called on adapter start for each active person.
	 * Does nothing if retentionDays is 0 (unlimited).
	 *
	 * @param {{name:string}} person
	 * @returns {Promise<void>}
	 */
	async _pruneOldData(person) {
		...
		try {
			...
			this.adapter.log.warn(`[Tracker:${person.name}] Prune: parse error: ${e.message}`);
			return;
		}
		...
		if (removed > 0) {
			...
			this.adapter.log.info(`[Tracker:${person.name}] Pruned ${removed} day(s) older than ${cutoffStr}`);
		} else {
			this.adapter.log.debug(`[Tracker:${person.name}] Prune: nothing to remove (cutoff: ${cutoffStr})`);
		}
	}
