...
<div id="header">
	...
	<div id="header-right">
		...
		<label style="margin-left:12px;user-select:none;">
			<input type="checkbox" id="showRoute" checked style="vertical-align:middle;"> Route anzeigen
		</label>
	</div>
</div>
<div id="map"></div>
<script>
	...
	function showDay(date) {
		...
		document.getElementById('headerInfo').textContent =
			(showRoute && coords.length > 1
				...
				: 'Nur letzter Punkt angezeigt (' + fmt(ts[ts.length-1]) + ')');
	}
  ...
  inpTo.addEventListener('change',   function() { if (inpFrom.value <= inpTo.value) showRange(inpFrom.value, inpTo.value); });
	...
	// Checkbox-Handler für Route anzeigen
	document.getElementById('showRoute').addEventListener('change', function() {
		// Nur Einzel-Tag-Modus beeinflussen
		showDay(inpTo.value);
	});
  setTimeout(function() {
