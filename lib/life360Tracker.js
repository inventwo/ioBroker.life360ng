...
class Tracker {
	...
	_loadConfig() {
		...
		this.config = {
			...
			pollInterval: (() => {
				const pi = Number(c.life360_polling_interval ?? 60); // ← Number() neu
	...
	<div id="header-right">
		...
		<input type="date" id="dateTo">
		   <label style="margin-left:12px;user-select:none;">
			   <input type="checkbox" id="showRoute" style="vertical-align:middle; accent-color:${color};"> Route
		   </label>
	</div>
</div>
<div id="map"></div>
<script>
  ...
  function showRange(from, to) {
    ...
    document.getElementById('headerInfo').textContent = feats.length + ' day(s) · ' + pts + ' points';
	...
	let showRouteState = localStorage.getItem('tracker_showRoute');
	...
	showRouteCheckbox.addEventListener('change', function() {
		// Persist state
		localStorage.setItem('tracker_showRoute', this.checked ? 'true' : 'false');
		// Only affect single-day mode
		showDay(inpTo.value);
		// Hide date pickers if route is enabled
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:sans-serif; background:${c.pageBg}; color:#eee; display:flex; flex-direction:column; height:100vh; }
  #header { padding:8px 14px; background:${c.headerBg}; display:flex; align-items:flex-start; justify-content:space-between; font-size:13px; border-bottom:2px solid ${c.headerBorder}; flex-wrap:wrap; gap:8px; }
