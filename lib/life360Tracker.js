...
function personShowRange(from,to){
  ...
  feats.forEach(function(feat){
    ...
    pl.on('mouseover',function(){var dh=document.getElementById('menuDayHighlight');if(!dh||!dh.checked){pl.closeTooltip();return;}_hoverOn(feat.date,false);});
    pl.on('mouseout',function(){_hoverOff();});
    pl.on('click',function(e){_blockMapClick=true;_hoverActive=false;var dh=document.getElementById('menuDayHighlight');if(dh&&dh.checked){if(_activeGroupKey===feat.date){_resetHighlight();return;}_activeGroupKey=feat.date;sessionStorage.setItem(ACTIVE_GROUP_KEY,feat.date);if(_linePopup){_linePopup.remove();_linePopup=null;}_applyGroupVisual(feat.date,false);_linePopup=L.popup({autoClose:false,closeOnClick:false}).setLatLng(e.latlng).setContent(feat.date).openOn(map);}});
    ...
    var addM=function(m){
      ...
      m.on('mouseover',function(){_hoverOn(feat.date,true);});
      m.on('mouseout',function(){_hoverOff();});
      m.on('click',function(){
...
function circleRender(feats,withDate){
  ...
  feats.forEach(function(feat){
    ...
    var addM=function(m){
      m._origStyle={fillOpacity:m.options.fillOpacity||1,opacity:m.options.opacity!=null?m.options.opacity:1,color:m.options.color||'#fff'};
      m.on('mouseover',function(){_hoverOn(grpKey,true);});
      m.on('mouseout',function(){_hoverOff();});
      m.on('click',function(){
        _blockMapClick=true;_hoverActive=false;
        var dh=document.getElementById('menuDayHighlight');
    ...
    if(showRoute){var pl=L.polyline(coords,{color:color,weight:C.routeWeight||4,opacity:C.routeOpacity||0.85}).addTo(map);pl.bindTooltip(feat.name+' - '+feat.date,{sticky:true,direction:'top',opacity:0.9});pl.on('mouseover',function(){var dh=document.getElementById('menuDayHighlight');if(!dh||!dh.checked){pl.closeTooltip();return;}_hoverOn(grpKey,false);});pl.on('mouseout',function(){_hoverOff();});pl.on('click',function(e){_blockMapClick=true;_hoverActive=false;var dh=document.getElementById('menuDayHighlight');if(dh&&dh.checked){if(_activeGroupKey===grpKey){_resetHighlight();return;}_activeGroupKey=grpKey;sessionStorage.setItem(ACTIVE_GROUP_KEY,grpKey);if(_linePopup){_linePopup.remove();_linePopup=null;}_applyGroupVisual(grpKey,false);_linePopup=L.popup({autoClose:false,closeOnClick:false}).setLatLng(e.latlng).setContent(feat.name+'<br>'+feat.date).openOn(map);}});grp.polyline=pl;layers.push(pl);bounds.push.apply(bounds,coords);}
