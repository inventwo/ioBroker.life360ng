...
if(inpFrom&&inpTo&&sortedDates.length){
  ...
  var RANGE_KEY='tracker_range_'+KEY;
  function defaultFromDate(){
    var d=new Date();d.setDate(d.getDate()-((D.defaultDays||1)-1));
    return d.toISOString().slice(0,10);
  }
  var storedFrom=localStorage.getItem(RANGE_KEY);
  var initFrom=storedFrom||defaultFromDate(),initTo=TODAY;
  ...
  inpFrom.addEventListener('change',function(){localStorage.setItem(RANGE_KEY,inpFrom.value);if(inpFrom.value<=inpTo.value)showRange(inpFrom.value,inpTo.value);});
...
var menuDayHighlight=document.getElementById('menuDayHighlight');if(menuDayHighlight){
  ...
  menuDayHighlight.addEventListener('change',function(){
    ...
    if(!this.checked&&_activeGroupKey)_resetHighlight();
  });
}
