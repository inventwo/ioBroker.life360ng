...
<script>
  ...
  // Persist map view across auto-refreshes; clears when tab/window is closed.
  var VIEW_KEY = 'tracker_view_${personKey}';
  var _savedView = null;
  try { _savedView = JSON.parse(sessionStorage.getItem(VIEW_KEY)); } catch(e) {}
  // Restore saved viewport immediately so Leaflet has a valid center on reload.
  if (_savedView) { map.setView([_savedView.lat, _savedView.lng], _savedView.zoom); }
  var _mapFitting = false;
  map.on('moveend', function() {
    if (_mapFitting) return;
    var ctr = map.getCenter();
    var v = { lat: ctr.lat, lng: ctr.lng, zoom: map.getZoom() };
    sessionStorage.setItem(VIEW_KEY, JSON.stringify(v));
    _savedView = v;
  });
  function autoFitBounds(bounds, opts) {
    if (_savedView) return;
    _mapFitting = true;
    map.fitBounds(bounds, opts);
    setTimeout(function() { _mapFitting = false; }, 600);
  }
  function autoSetView(latlng, zoom) {
    if (_savedView) return;
    _mapFitting = true;
  ...
  // Fit-button: resets saved view and re-fits to current data
  var FitControl = L.Control.extend({
    onAdd: function() {
      var container = L.DomUtil.create('div', 'leaflet-bar leaflet-control');
      var a = L.DomUtil.create('a', '', container);
      a.href = '#';
      a.title = 'Auf Marker zoomen';
      ...
      L.DomEvent.on(a, 'click', function(e) {
        ...
        _savedView = null;
        var route = document.getElementById('menuRoute');
        var from = document.getElementById('dateFrom');
        var to = document.getElementById('dateTo');
        if (route && route.checked) { showRange(from.value, to.value); } else { showDay(to.value); }
      });
      return container;
    }
  });
...
var cb = document.getElementById('menuPlaces');
...
var cb = document.getElementById('menuMyPlaces');
if (!cb || !cb.checked) return;
...
myFlagLayers.push(L.marker([p.lat, p.lng], { icon: flagIcon(MYPLACES_COLOR, MYPLACES_FLAG_SIZE) }).bindPopup('<b>' + p.name + '</b>').addTo(map));
	...
	function showDay(date) {
		...
		const showRoute = document.getElementById('menuRoute')?.checked;
		...
		if (showRoute && coords.length > 1) {
			autoFitBounds(L.latLngBounds(coords), { padding:[30,30] });
		} else {
			autoSetView(coords[coords.length-1], 16);
		}
  ...
  if (location.hash) {
	...
	function showRange(from, to) {
		...
		if (bounds.length > 0) autoFitBounds(L.latLngBounds(bounds), { padding:[30,30] });
...
<script>
  ...
  // Persist map view across auto-refreshes; clears when tab/window is closed.
  var VIEW_KEY = 'tracker_view_circle';
  var _savedView = null;
  try { _savedView = JSON.parse(sessionStorage.getItem(VIEW_KEY)); } catch(e) {}
  // Restore saved viewport immediately so Leaflet has a valid center on reload.
  if (_savedView) { map.setView([_savedView.lat, _savedView.lng], _savedView.zoom); }
...
myFlagLayers.push(L.marker([p.lat, p.lng], { icon: flagIcon(MYPLACES_COLOR, MYPLACES_FLAG_SIZE) }).bindPopup('<b>' + p.name + '</b>').addTo(map));
	...
	function renderFeatures(feats, withDateOnly) {
		...
		if (showRoute && bounds.length > 0) {
			autoFitBounds(L.latLngBounds(bounds), { padding:[30,30] });
			...
			if (lastPoints.length === 1) {
				autoSetView(lastPoints[0], 16);
			} else {
				autoFitBounds(L.latLngBounds(lastPoints), { padding:[30,30] });
			}
		}
	}
