	...
	_loadConfig() {
		...
		this.config = {
			...
			mapColors: {
				...
				legendEnabled: c.tracker_map_legend ?? true,
				mapSizeEnabled: c.tracker_map_size ?? false,
				personFooterEnabled: c.tracker_person_footer ?? true,
				circleFooterEnabled: c.tracker_circle_footer ?? true,
				showPlaces: c.tracker_show_places ?? false,
				placesColor: c.tracker_places_color || "#e67e22",
				placesFlagSize: c.tracker_places_flag_size ?? 1.0,
				showMyPlaces: c.tracker_show_myplaces ?? false,
				myPlacesColor: c.tracker_myplaces_color || "#27ae60",
	...
	<div id="header-right">
		...
		<input type="date" id="dateTo">
		   <label class="route-checkbox-label">
			   <input type="checkbox" id="showRoute"> ${this._t("route")}
		   </label>
${c.showPlaces ? `		   <label class="route-checkbox-label"><input type="checkbox" id="showPlaces" style="accent-color: ${c.placesColor}"> ${this._t("places")}</label>` : ""}
${c.showMyPlaces ? `		   <label class="route-checkbox-label"><input type="checkbox" id="showMyPlaces" style="accent-color: ${c.myPlacesColor}"> ${this._t("myPlaces")}</label>` : ""}
	</div>
</div>
<div id="map"></div>
<div id="footer"${c.personFooterEnabled ? "" : ' style="display:none"'}>
${
	c.legendEnabled
...
${c.mapSizeEnabled ? `  <span style="margin-left:auto">${this._t("mapSize")}: __MAPSIZE__</span>` : ``}
</div>
...
myFlagLayers.push(L.marker([p.lat, p.lng], { icon: flagIcon(MYPLACES_COLOR, MYPLACES_FLAG_SIZE) }).bindPopup('<b>' + p.name + '</b>').addTo(map));
  ...
  if (location.hash) {
	...
	if (!showRouteCheckbox.checked) {
		...
		var legInit = document.getElementById('legend'); if (legInit) legInit.style.display = 'none';
...
if (SHOW_PLACES) {
var cbP = document.getElementById('showPlaces');
cbP.checked = localStorage.getItem(PLACES_STORAGE_KEY) !== 'false';
...
if (SHOW_MYPLACES) {
var cbM = document.getElementById('showMyPlaces');
cbM.checked = localStorage.getItem(MYPLACES_STORAGE_KEY) !== 'false';
cbM.addEventListener('change', function() {
...
<div id="map"></div>
<div id="footer"${c.circleFooterEnabled ? "" : ' style="display:none"'}>
${
	c.legendEnabled
		? `  <div id="legend" style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">
  <span class="leg-entry">
