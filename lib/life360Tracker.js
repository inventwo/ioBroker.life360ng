...
map.on('click',function(){if(_blockMapClick){_blockMapClick=false;return;}if(_activeGroupKey)_resetHighlight();});
...
function personShowRange(from,to){
  ...
  feats.forEach(function(feat){
    ...
    var addM=function(m){
      ...
      m.on('click',function(){_blockMapClick=true;_highlightGroup(feat.date);});
...
function circleRender(feats,withDate){
  ...
  feats.forEach(function(feat){
    ...
    var addM=function(m){
      m._origStyle={fillOpacity:m.options.fillOpacity||1,opacity:m.options.opacity!=null?m.options.opacity:1,color:m.options.color||'#fff'};
      m.on('click',function(){_blockMapClick=true;_highlightGroup(grpKey);});
      grp.markers.push(m);layers.push(m);
    };
    if(showRoute){var pl=L.polyline(coords,{color:color,weight:C.routeWeight||4,opacity:C.routeOpacity||0.85}).addTo(map);grp.polyline=pl;layers.push(pl);bounds.push.apply(bounds,coords);}
