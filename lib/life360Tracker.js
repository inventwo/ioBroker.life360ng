...
<script>
  ...
  // Persist map view (center+zoom) in sessionStorage across auto-refreshes.
  // sessionStorage is cleared when the tab/window is closed → fresh fit on next open.
  ...
  map.on('moveend', function() {
    ...
    _savedView = { lat: ctr.lat, lng: ctr.lng, zoom: map.getZoom() };
  });
  function autoFitBounds(bounds, opts) {
    _mapFitting = true;
    if (_savedView) {
      map.setView([_savedView.lat, _savedView.lng], _savedView.zoom);
    } else {
