		...
		for (const person of activePeople) {
			...
			if (existingMonth?.val) {
				try {
					...
					this.adapter.log.warn(`[Tracker:family] Monthly parse error for ${person.name}: ${e.message}`);
				}
			}
		}
...
<script>
  ...
  let layers = [];
	...
	function pinIcon(color) {
		return L.divIcon({
			className: '',
			html: '<svg xmlns="http://www.w3.org/2000/svg" width="28" height="36" viewBox="0 0 28 36"><path d="M14 0C6.27 0 0 6.27 0 14c0 9.33 14 22 14 22S28 23.33 28 14C28 6.27 21.73 0 14 0z" fill="' + color + '" stroke="#fff" stroke-width="2"/><circle cx="14" cy="14" r="5" fill="#fff"/></svg>',
			iconSize: [28, 36],
			iconAnchor: [14, 36],
			popupAnchor: [0, -36]
		});
...
<script>
  ...
  setTimeout(function() {
	 ...
	 * Called on adapter start when tracker_delete_family_html_now is set,
	 * or when individual persons have clearRecordings=true in their config.
	 *
