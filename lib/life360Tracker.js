...
class Tracker {
	...
	async init() {
		...
		this._lastPurgeDate = this._localDate();
	 ...
	 * Using getFullYear/getMonth/getDate avoids the UTC-offset issue of toISOString().
	 *
	 * @returns {string} Local date string, e.g. "2026-05-20"
	 */
	_localDate() {
		const d = new Date();
	...
	_loadConfig() {
		...
		this.config = {
			...
			minDistance: c.tracker_min_distance ?? 10,
			pollInterval: (() => {
			...
			mapColors: {
				...
				showMyPlaces: c.enable_myplaces !== false && (c.tracker_show_myplaces ?? false),
	...
	async _syncPeople() {
		let objects;
		try {
			objects = await this.adapter.getObjectViewAsync("system", "channel", {
				startkey: `${this.config.namespace}.people.`,
				endkey: `${this.config.namespace}.people.\u9999`,
			});
		} catch (e) {
			this.adapter.log.debug(`[Tracker] Could not sync persons from DB: ${e.message}`);
			return;
		}
		...
		// Remove people that are no longer in Life360
		const removedPeople = people.filter(p => !knownNames.includes(p.name));
		if (removedPeople.length > 0) {
			for (const person of removedPeople) {
				const safe = person.name.toLowerCase().replace(/\s+/g, "_");
				const trackerObjId = `${this.config.namespace}.tracker.${safe}`;
				try {
					await this.adapter.delObjectAsync(trackerObjId, { recursive: true });
					this.adapter.log.info(`[Tracker] Removed tracker objects for ${person.name}`);
				} catch (e) {
					this.adapter.log.debug(`[Tracker] No tracker objects to remove for ${person.name}: ${e.message}`);
				}
			}
			people = people.filter(p => knownNames.includes(p.name));
			changed = true;
		}
	...
	_generateHTML(person, fc, includeRoute = true) {
		...
		const resolvedName = iobHelpers.resolveDisplayName(person.name, this.config.aliases);
	...
	<div id="header-right">
		...
		<div id="hamMenu">
			...
			<div id="hamPanel">
				...
				<label><input type="checkbox" id="menuAutoRefresh"> ${this._t("autoRefresh")}</label>
				<label><input type="checkbox" id="menuLiveFollow"> ${this._t("liveFollow")}</label>
...
<script src="/${ns}/tracker/tracker-map.js?v=${this._startupTs}"></` +
	...
	async _writeSharedFiles() {
		const ns = this.config.namespace;
		// Ensure the namespace object has type "meta" before writing files,
		// because writeFile() requires the target namespace to be a meta object.
		// This can be called before _ensureChannels(), so we must guarantee it here.
		await this.adapter.extendObjectAsync(ns, {
			type: "meta",
			common: { name: "User files for life360ng", type: "meta.user" },
			native: {},
...
var FEAT=D.features||[];
function _localDate(){var d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');}
var TODAY=D.today||_localDate();
var SEL=D.selDate||TODAY;
var KEY=D.personKey||'map';
var LIVE_FOLLOW_KEY='tracker_livefollow_'+KEY;
var liveFollowEnabled=localStorage.getItem(LIVE_FOLLOW_KEY)==='true';
...
function forceFitLatLngs(latlngs,opts){
  if(!latlngs||!latlngs.length)return;
  clearSavedView();
  _fitting=true;
  if(latlngs.length===1){map.setView(latlngs[0],(opts&&opts.maxZoom)||16);}
  else{map.fitBounds(L.latLngBounds(latlngs),opts||{padding:[40,40],maxZoom:16});}
  setTimeout(function(){_fitting=false;},700);
}
function liveFollowPointForPerson(personName,from,to){
  var list=FEAT.filter(function(f){
    if(!f.coords||!f.coords.length)return false;
    if(D.mode==='circle'&&personName&&f.name!==personName)return false;
  ...
  list.sort(function(a,b){
    var at=(a.timestamps&&a.timestamps.length)?a.timestamps[a.timestamps.length-1]:0;
    var bt=(b.timestamps&&b.timestamps.length)?b.timestamps[b.timestamps.length-1]:0;
    if(at!==bt)return at-bt;
    return String(a.date).localeCompare(String(b.date));
  });
  var last=list[list.length-1];
  return last.coords[last.coords.length-1]||null;
}
function getAutoRefreshKey(){return 'tracker_autorefresh_'+KEY;}
function setAutoRefreshEnabled(enabled){
  localStorage.setItem(getAutoRefreshKey(),enabled?'true':'false');
  var cb=document.getElementById('menuAutoRefresh');
  if(cb)cb.checked=!!enabled;
}
function getLiveFollowPersonName(){
  if(D.mode!=='circle')return null;
  if(typeof _focusPersonName!=='undefined'&&_focusPersonName)return _focusPersonName;
  var stored=localStorage.getItem('tracker_focus_'+KEY);
  if(stored)return stored;
  if(typeof visiblePeople==='undefined')return null;
  var names=Object.keys(visiblePeople).filter(function(n){return visiblePeople[n];});
  return names.length===1?names[0]:null;
}
function pickLiveFollowPerson(){
  if(D.mode!=='circle')return null;
  var current=getLiveFollowPersonName();
  if(current)return current;
  if(typeof visiblePeople==='undefined')return null;
  var names=Object.keys(visiblePeople).filter(function(n){return visiblePeople[n];});
  if(!names.length&&Array.isArray(D.people)&&D.people.length)names=[D.people[0].name];
  return names[0]||null;
}
function applyLiveFollow(){
  if(!liveFollowEnabled)return;
  var from=document.getElementById('dateFrom')?document.getElementById('dateFrom').value:null;
  var to=document.getElementById('dateTo')?document.getElementById('dateTo').value:null;
  var personName=getLiveFollowPersonName();
  var pt=liveFollowPointForPerson(personName,from,to);
  if(!pt)return;
  clearSavedView();
  _fitting=true;
  var z=map.getZoom();
  if(!z||z<15)z=16;
  map.setView(pt,z,{animate:true});
  setTimeout(function(){_fitting=false;},700);
}
...
var _focusPersonName=null;
var _focusPending=false;
function _setPersonCheckboxState(personName,checked){
  document.querySelectorAll('.personToggle').forEach(function(cb){
    cb.checked=checked?cb.value===personName:true;
    visiblePeople[cb.value]=checked?cb.value===personName:true;
    localStorage.setItem('tracker_person_'+cb.value+'_'+KEY,visiblePeople[cb.value]?'true':'false');
  });
}
function circleRender(feats,withDate){
  clearLayers();if(!feats.length)return;
  var showRoute=document.getElementById('menuRoute')&&document.getElementById('menuRoute').checked;
  var bounds=[],focusBounds=[],maxByPerson={};
  ...
  feats.forEach(function(feat){
    ...
    if(_focusPending&&_focusPersonName===feat.name){focusBounds.push.apply(focusBounds,coords);}
  ...
  if(_focusPending){
    if(focusBounds.length){forceFitLatLngs(focusBounds,{padding:[45,45],maxZoom:16});}
    _focusPending=false;
  }else if(showRoute&&bounds.length){autoFit(L.latLngBounds(bounds),{padding:[30,30]});}
...
// ── Circle: person toggles / focus mode ──────────────────
if(D.mode==='circle'){
  function refreshCircle(){
    if(menuRoute&&menuRoute.checked){if(inpFrom&&inpTo)showRange(inpFrom.value,inpTo.value);}
    else{if(inpTo)showDay(inpTo.value);}
  }
  function focusPerson(personName){
    if(!personName)return;
    _focusPersonName=personName;
    localStorage.setItem('tracker_focus_'+KEY,personName);
    _focusPending=true;
    _resetHighlight();
    _setPersonCheckboxState(personName,true);
    if(menuRoute){
      menuRoute.checked=true;
      localStorage.setItem(D.storageKey||'tracker_showRoute','true');
      if(typeof applyRoute==='function')applyRoute(true);
    }
    clearSavedView();
    refreshCircle();
    if(liveFollowEnabled)setTimeout(applyLiveFollow,150);
  }
  document.querySelectorAll('.personToggle').forEach(function(cb){
    cb.checked=localStorage.getItem('tracker_person_'+cb.value+'_'+KEY)!=='false';
    visiblePeople[cb.value]=cb.checked;
    cb.addEventListener('change',function(){
      _focusPersonName=null;
      localStorage.removeItem('tracker_focus_'+KEY);
      _focusPending=false;
  ...
  document.querySelectorAll('.legend-item').forEach(function(label){
    var cb=label.querySelector('.personToggle');
    if(!cb)return;
    label.title='Klick auf den Namen: Person fokussieren';
    label.addEventListener('click',function(e){
      if(e.target&&e.target.classList&&e.target.classList.contains('personToggle'))return;
      e.preventDefault();
      e.stopPropagation();
      focusPerson(cb.value);
    });
  });
  ...
  if(showAllBtn){showAllBtn.addEventListener('click',function(){
    _focusPersonName=null;localStorage.removeItem('tracker_focus_'+KEY);_focusPending=false;_resetHighlight();clearSavedView();
    document.querySelectorAll('.personToggle').forEach(function(cb){cb.checked=true;visiblePeople[cb.value]=true;localStorage.setItem('tracker_person_'+cb.value+'_'+KEY,'true');});
    refreshCircle();
  });}
  if(hideAllBtn){hideAllBtn.addEventListener('click',function(){
    _focusPersonName=null;localStorage.removeItem('tracker_focus_'+KEY);_focusPending=false;_resetHighlight();
    document.querySelectorAll('.personToggle').forEach(function(cb){cb.checked=false;visiblePeople[cb.value]=false;localStorage.setItem('tracker_person_'+cb.value+'_'+KEY,'false');});
    refreshCircle();
  });}
}

// ── Live follow ───────────────────────────────────────────
var menuLiveFollow=document.getElementById('menuLiveFollow');
if(menuLiveFollow){
  menuLiveFollow.checked=liveFollowEnabled;
  menuLiveFollow.addEventListener('change',function(){
    liveFollowEnabled=this.checked;
    localStorage.setItem(LIVE_FOLLOW_KEY,this.checked?'true':'false');
    if(this.checked){
      setAutoRefreshEnabled(true);
      if(D.mode==='circle'){
        var personName=pickLiveFollowPerson();
        if(personName&&typeof focusPerson==='function')focusPerson(personName);
      }
      setTimeout(applyLiveFollow,150);
    }
  });
}

// ── Auto-refresh ──────────────────────────────────────────
var menuAutoRefresh=document.getElementById('menuAutoRefresh');
var autoRefreshKey=getAutoRefreshKey();
var autoRefreshEnabled=localStorage.getItem(autoRefreshKey)!=='false';
if(menuAutoRefresh){
  menuAutoRefresh.checked=autoRefreshEnabled;
  menuAutoRefresh.addEventListener('change',function(){
    localStorage.setItem(autoRefreshKey,this.checked?'true':'false');
    if(this.checked){
...
if(liveFollowEnabled)setTimeout(applyLiveFollow,250);
if(D.refresh&&autoRefreshEnabled){
  setTimeout(function(){
    if(localStorage.getItem(autoRefreshKey)==='false')return;
    var url=new URL(window.location.href);
    url.searchParams.set('_t',Date.now());
    window.location.replace(url.toString());
  },D.refresh*1000);
