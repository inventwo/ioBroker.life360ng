...
class Tracker {
	...
	async init() {
		...
		// _ensureConfigStates writes adapter.config values into states on every start.
		// _loadConfigFromStates is NOT called here — admin config always wins on restart.
		// The state tracker.config.enabled can still be changed at runtime via onStateChange.
	...
	_loadConfig() {
		...
		this.adapter.log.debug(`Tracker _loadConfig trackerenabled=${c.trackerenabled}`);
		this.config = {
	 ...
	 *
	 * @returns {Array<{id:string, value:any, meta:object}>}
	 */
	_configStateDefs() {
		return [
			{
	 ...
	 * syncs current adapter.config values into them, and subscribes to changes.
	 * Admin config always wins on adapter start — state value is overwritten.
	 *
	 * @returns {Promise<void>}
	 */
	async _ensureConfigStates() {
		...
		for (const def of this._configStateDefs()) {
			...
			// Always write admin config value into state on start
	 ...
	 *
	 * @param {string} name
	 * @returns {{geojson:string, allTime:string, url:string, filePath:string}}
	 */
	_getPaths(name) {
		const month = String(new Date().getMonth() + 1).padStart(2, "0");
		const safe = name.toLowerCase().replace(/\s+/g, "_");
