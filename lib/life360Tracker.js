...
<script>
  ...
  // userPanned: true = user has manually moved/zoomed → skip auto-fit on refresh
  // Stored in sessionStorage so it resets when the tab/window is closed
  var USER_PANNED_KEY = 'tracker_userpanned_${personKey}';
  var _mapFitting = false;
  var userPanned = sessionStorage.getItem(USER_PANNED_KEY) === 'true';
  map.on('moveend', function() {
    if (!_mapFitting) {
      userPanned = true;
      sessionStorage.setItem(USER_PANNED_KEY, 'true');
    }
  });
  function autoFitBounds(bounds, opts) {
    ...
    setTimeout(function() { _mapFitting = false; }, 500);
  }
...
<script>
  ...
  // userPanned: true = user has manually moved/zoomed → skip auto-fit on refresh
  // Stored in sessionStorage so it resets when the tab/window is closed
  var USER_PANNED_KEY = 'tracker_userpanned_circle';
  var _mapFitting = false;
  var userPanned = sessionStorage.getItem(USER_PANNED_KEY) === 'true';
  map.on('moveend', function() {
    if (!_mapFitting) {
