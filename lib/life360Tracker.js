...
class Tracker {
	...
	/**
	 * Loads the configuration from the adapter config
	 */
	_loadConfig() {
	...
	async _ensureChannels(name) {
		...
		// meta object required for writeFile
		...
		// tracker.<Name>
		await this.adapter.setObjectNotExistsAsync(`${ns}.tracker.${name}`, {
			type: "channel",
			common: { name },
			native: {},
		});

		// tracker.<Name>.allTime
		await this.adapter.setObjectNotExistsAsync(`${ns}.tracker.${name}.allTime`, {
			type: "channel",
			common: { name: "All time" },
			native: {},
		});

		// tracker.<Name>.currentYear
		await this.adapter.setObjectNotExistsAsync(`${ns}.tracker.${name}.currentYear`, {
			type: "channel",
			common: { name: "Current year" },
			native: {},
		});

		// tracker.<Name>.currentYear.<MM>
		await this.adapter.setObjectNotExistsAsync(`${ns}.tracker.${name}.currentYear.${month}`, {
			type: "channel",
	...
	async onStateChange(id, state) {
		...
		// Person lookup via UUID map (no name matching required)
	...
	async _updateFamilyMap() {
		...
		for (const person of activePeople) {
			...
			try {
				...
				// Enrich each feature with person color (if not already set)
	...
	_generateHTML(person, fc) {
		...
		const refresh = this.config.pollInterval + 10; // 10s buffer on top of poll interval
		...
		// Pre-select today or the most recent day
		const today = new Date().toISOString().slice(0, 10);
		const dates = features.map(f => f.properties.date);
		const selDate = dates.includes(today) ? today : dates[dates.length - 1];

		// Serialize features as JSON for the client
		const featuresJSON = JSON.stringify(
			features.map(f => ({
				date: f.properties.date,
				coords: f.geometry.coordinates.map(coord => [coord[1], coord[0]]), // [lat, lon] for Leaflet
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  select { background:${c.headerBg}; color:#eee; border:1px solid ${c.headerBorder}; border-radius:4px; padding:3px 6px; font-size:13px; cursor:pointer; }
  #map { flex:1; }
</style>
</head>
<body>
...
<script>
  ...
  // Populate dropdown (newest date first)
  const sel = document.getElementById('daySelect');
  FEATURES.slice().reverse().forEach(function(f) {
    const opt = document.createElement('option');
    opt.value = f.date;
    opt.textContent = f.date;
    if (f.date === SEL_DATE) opt.selected = true;
    sel.appendChild(opt);
  });
  sel.addEventListener('change', function() { showDay(this.value); });

  if (FEATURES.length > 0) showDay(SEL_DATE);
  else map.setView([53.0, 9.0], 10);
</` +
			`script>
</body>
</html>`
	...
	_generateFamilyHTML(personFCs) {
		...
		// Merge all features from all persons
		const allFeatures = [];
...
<script>
  ...
  // Populate dropdown (newest date first)
  const allDates = [...new Set(FEATURES.map(f => f.date))].sort().reverse();
  const sel = document.getElementById('daySelect');
  allDates.forEach(function(d) {
    const opt = document.createElement('option');
    opt.value = d;
    opt.textContent = d;
    if (d === SEL_DATE) opt.selected = true;
    sel.appendChild(opt);
  });
  sel.addEventListener('change', function() { showDay(this.value); });
  ...
  if (FEATURES.length > 0) showDay(SEL_DATE);
	 ...
	 * @param {{name:string, color:string}} [personConfig]
	 * @returns {object} Feature for today
	 */
	_getTodayFeature(fc, personConfig) {
		const today = new Date().toISOString().slice(0, 10);
		...
		if (!feature) {
			feature = {
				...
				properties: {
					...
					color: personConfig?.color || "#4a90e2",
					pointCount: 0,
					startTime: null,
					endTime: null,
				},
	 ...
	 *
	 * @param {string} name - Display name or family identifier
	 * @returns {{geojson:string, allTime:string, url:string, filePath:string}} Object with relative state paths and file path
	 */
	_getPaths(name) {
		const now = new Date();
		const month = String(now.getMonth() + 1).padStart(2, "0");
		const base = `tracker.${name}.currentYear.${month}`;
		const safe = name.toLowerCase().replace(/\s+/g, "_");
		return {
			geojson: `${base}.geojson`,
			allTime: `tracker.${name}.allTime.geojson`,
	 ...
	 * @returns {string} Full HTTP URL to the person's HTML map
	 */
	_buildUrl(person) {
	 ...
	 *
	 * @returns {string} Full HTTP URL to the family map HTML file
	 */
	_buildFamilyUrl() {
		const ip = this._getLocalIP();
	 ...
	 *
	 * @returns {string} First external IPv4 address found, or 127.0.0.1 as fallback
	 */
	_getLocalIP() {
		try {
			const os = require("os");
	 ...
	 * @returns {Promise<string>} UUID segment of the person (part of the ioBroker object ID after .people.)
	 */
	 ...
	 * @param {number} lat2
	 * @param {number} lon2
	 * @returns {number} Distance in meters between two GPS coordinates (Haversine formula)
	 */
	 ...
	 * @returns {string} Darkened hex color value in #rrggbb format
	 */
	 ...
	 * @returns {Promise<void>} Resolved after the object has been created and the state value has been set
	 */
