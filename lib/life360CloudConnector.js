...
const Life360APIMaxRetries = 2;     //  Max retries for http-requests against the Life360 API
...
exports.connectLife360 = function(username, password, phone, countryCode) {
    return new Promise((resolve, reject) => {
        ...
        if (phone) {
            ...
            formData.countryCode = countryCode || 1;
        } else {
        ...
        request(options)
            .then(response => {
                ...
                try {
                    ...
                    logger("error", `Life360 login failed - HTTP ${response.statusCode}, body: ${String(response.body).substring(0, 300)}`);
                ...
                if ((response.statusCode !== 200) || !body["access_token"]) {
                    ...
                    logger("error", `Connection established but failed to authenticate. HTTP ${response.statusCode}, response: ${JSON.stringify(body)}`);
        ...
        do {
            ...
            if (response.body && response.body.members) {
                ...
                if (response.body.members.length > 0) {
                    logger("debug", `First member keys: ${Object.keys(response.body.members[0]).join(", ")}`);
                    logger("debug", `First member full data: ${JSON.stringify(response.body.members[0])}`);
                }
