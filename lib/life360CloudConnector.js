 ...
 * @returns {Promise<{statusCode: number, body: object|string}>} Response with parsed JSON body.
 */

/**
 * Checks whether an API response was blocked by Cloudflare.
 * Returns a short human-readable reason string, or null if not blocked.
 *
 * @param {{statusCode: number, body: object|string}} response - The httpsRequest response.
 * @returns {string|null} Human-readable block reason, or null if not blocked.
 */
function detectCloudflareBlock(response) {
	const bodyStr = typeof response.body === "string" ? response.body : "";
	if (
		response.statusCode === 403 ||
		response.statusCode === 503 ||
		bodyStr.includes("Cloudflare") ||
		bodyStr.includes("cf-wrapper")
	) {
		return `Life360 API blocked by Cloudflare (HTTP ${response.statusCode}). Your server's IP may be rate-limited or flagged. Try increasing the poll interval or check your Life360 token.`;
	}
	return null;
}
...
// Credentials are set via exports.setAdapter() from adapter.config
let life360_username = "";
let life360_token = "";

/** Set to true once the "all place sources returned 0" info message has been logged, to avoid spam. */
let placesUnavailableLogged = false;
...
/**
 * Notify the Life360 cloud connector about finishing an online operation.
 */
 ...
 *
 * @param {string} [username] Life360 username.
 */
		...
		if (!exports.is_connected()) {
			...
			exports
				...
				.then(auth_new => {
					...
					objTimeoutConnection = adapter.setTimeout(
						() => {
							exports.disconnect();
						},
...
exports.disconnect = async function () {
	if (adapter) {
		adapter.clearTimeout(objTimeoutConnection);
	}
	...
	return new Promise((resolve, reject) => {
		...
		httpsRequest(options)
			.then(response => {
				...
				} else if (response.body.circles.length === 0) {
...
/**
 * Returns details for a Life360 circle identified by the circle's id.
 *
 * @param {object} auth_in
...
/**
 * Returns an array containing a circle's members.
 *
 * @param {object} circle_in
 */
exports.getCircleMembers = function (circle_in) {
	...
	} else if (circle_in.members.length === 0) {
...
exports.disablePolling = function () {
	if (objIntervalPoll) {
		adapter.clearTimeout(objIntervalPoll);
		objIntervalPoll = null;
 ...
 *
 * @param {function(boolean|Error, object|null): void} callback
 */
exports.setupPolling = function (callback) {
	const polling_interval = Number(adapter.config.life360_polling_interval);

	if (polling_interval < min_polling_interval) {
		logger("error", `Polling interval should be greater than ${min_polling_interval} seconds.`);
		return false;
	}
	...
	function schedulePoll() {
		exports.pollAsync(callback, () => {
			objIntervalPoll = adapter.setTimeout(schedulePoll, polling_interval * 1000);
		});
	}
	schedulePoll();
 ...
 *
 * @param {function(boolean|Error, object|null): void} callback The callback function.
 * @param {function(): void} done
 */
exports.pollAsync = function (callback, done) {
...
async function pollLife360DataAsync() {
	...
	try {
		...
		do {
			...
			try {
				...
				if (counter <= Life360APIDataMaxRetries) {
					...
					await adapter.delay(Life360RetryDelay);
				}
			}
		} while (counter <= Life360APIDataMaxRetries && !auth_in);

		if (!auth_in) {
			logger("error", `Failed to connect or to login after ${counter} attempt(s). Aborting ...`);
			throw lastError;
		}
		...
		for (let c in circles) {
			...
			// Save places already embedded in the v4 circles response (if any) before overwriting.
			// The v4 /circles endpoint may embed places under "places" (array) or "place" (singular key).
			const v4CirclePlaces = Array.isArray(circle.places)
				? [...circle.places]
				: Array.isArray(circle.place)
					? [...circle.place]
					: [];
			...
			// Fallback 1: v3 places endpoint returned nothing but v4 circles response had embedded places.
			if (Array.isArray(circlePlaces) && circlePlaces.length === 0 && v4CirclePlaces.length > 0) {
				circle.places = v4CirclePlaces;
				logger(
					"debug",
					`v3 places endpoint returned 0 for circle ${circle.id}; using ${v4CirclePlaces.length} place(s) from v4 circles response as fallback.`,
				);
			} else if (Array.isArray(circlePlaces) && circlePlaces.length === 0 && v4CirclePlaces.length === 0) {
				// Fallback 2: both v3 and embedded v4 places are empty – try the v4 places endpoint directly.
				await adapter.delay(300);
				const circlePlacesV4 = await exports.getCirclePlacesV4Async(auth_in, circle.id);
				circle.places = circlePlacesV4;
				if (circlePlacesV4.length > 0) {
					logger(
						"debug",
						`v3 and embedded v4 places returned 0 for circle ${circle.id}; using ${circlePlacesV4.length} place(s) from v4 places endpoint as fallback.`,
					);
				} else if (!placesUnavailableLogged) {
					placesUnavailableLogged = true;
					logger(
						"info",
						`All place sources (v3, embedded v4, v4 endpoint) returned 0 places for circle ${circle.id}. Life360 may not expose places for this account via the API.`,
					);
				}
			} else {
				circle.places = Array.isArray(circlePlaces) ? circlePlaces : [];
			}
 ...
 *
 * @param {object} auth_in The auth object.
 */
	...
	try {
		...
		do {
			...
			if (response.body && response.body.circles) {
				...
				const cfBlock = detectCloudflareBlock(response);
				if (cfBlock) {
					logger("warn", `[circles] ${cfBlock}`);
					break; // Do not retry immediately – further requests would worsen the block
				} else {
					logger(
						"warn",
						`Unexpected circles response (HTTP ${response.statusCode}): ${typeof response.body === "string" ? response.body.substring(0, 200) : JSON.stringify(response.body)}`,
					);
				}
			}
		...
		logger("error", `Failed to retrieve circles: ${error}`);
		stopOnlineOperation();
	}
};
 ...
 *
 * @param {object} auth_in The auth object.
 * @param {string} circleId The id of a Life360 circle.
 */
	...
	try {
		...
		do {
			...
			if (response.body && response.body.members) {
				...
				const cfBlock = detectCloudflareBlock(response);
				if (cfBlock) {
					logger("warn", `[members] ${cfBlock}`);
					break; // Do not retry immediately – further requests would worsen the block
				} else {
					logger(
						"warn",
						`Unexpected members response (HTTP ${response.statusCode}): ${typeof response.body === "string" ? response.body.substring(0, 200) : JSON.stringify(response.body)}`,
					);
				}
			}
		} while (counter <= Life360APIDataMaxRetries && obj === undefined);
	...
	try {
		...
		do {
			...
			if (response.body && response.body.places) {
				...
				const cfBlock = detectCloudflareBlock(response);
				if (cfBlock) {
					logger("warn", `[places] ${cfBlock}`);
					break; // Do not retry immediately – further requests would worsen the block
				} else {
					logger(
						"warn",
						`Unexpected places response (HTTP ${response.statusCode}): ${typeof response.body === "string" ? response.body.substring(0, 200) : JSON.stringify(response.body)}`,
					);
				}
			}
		} while (counter <= Life360APIDataMaxRetries && obj === undefined);
		...
		if (obj.length === 0) {
			logger("debug", `v3 places endpoint returned 0 places for circle ${circleId}.`);
		}
...
/**
 * Tries to retrieve the Life360 circle's places via the v4 API endpoint.
 * Used as a fallback when the v3 endpoint and embedded v4 circles data both return 0 places.
 *
 * @param {object} auth_in The auth object.
 * @param {string} circleId The id of a Life360 circle.
 * @returns {Promise<object[]>} Array of place objects, or empty array on failure.
 */
exports.getCirclePlacesV4Async = async function (auth_in, circleId) {
	if (!auth_in) {
		auth_in = auth;
	}
	...
	logger("debug", `[v4] Trying places endpoint at ${URL}`);

	try {
		startOnlineOperation();

		const response = await httpsRequest(options);
		// v4 might return { places: [...] } or { place: [...] } (singular key)
		if (response.body && Array.isArray(response.body.places)) {
			logger("debug", `[v4] Retrieved ${response.body.places.length} place(s).`);
			stopOnlineOperation();
			return response.body.places;
		}
		if (response.body && Array.isArray(response.body.place)) {
			logger("debug", `[v4] Retrieved ${response.body.place.length} place(s) (singular key).`);
			stopOnlineOperation();
			return response.body.place;
		}
		const cfBlock = detectCloudflareBlock(response);
		if (cfBlock) {
			logger("warn", `[places/v4] ${cfBlock}`);
		} else {
			logger(
				"debug",
				`[v4] Unexpected places response (HTTP ${response.statusCode}): ${typeof response.body === "string" ? response.body.substring(0, 200) : JSON.stringify(response.body).substring(0, 200)}`,
			);
		}
		...
		logger("debug", `[v4] Failed to retrieve places: ${error}`);
		stopOnlineOperation();
		return [];
	}
};
