...
async function pollLife360DataAsync() {
	...
	try {
		...
		for (let c in circles) {
			...
			const v4CirclePlaces = Array.isArray(circle.places) ? [...circle.places] : [];

			await adapter.delay(300);
			const circlePlaces = await exports.getCirclePlacesAsync(auth_in, circle.id);

			// If the v3 places endpoint returned nothing but the v4 circles response already contained
			// places, use the v4 data as a fallback. This fixes accounts where the v3 /places endpoint
			// returns an empty array even though places exist (the v4 /circles response embeds them).
